...
// 可以做一个白名单 指定 ***能用 *** 不能用
// 前端用到的js 都不要 都通过后端 Node 去require
...
const options = {
    ...
    //告诉webpack 以 node试用的方式 处理动态导入 并且还会通知vue编译组件时成成服务端的代码
    target: 'node',
    ...
    output: {
        ...
        publicPath: '/',
        //此处告知 server bundle 使用 Node 风格导出模块(Node-style exports)  用commonjs2的规范编译
...
const _options = _.assign(options, conf.server);
